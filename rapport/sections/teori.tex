...
\draw(0,0) to[sV, l={$V_{in}(t)$}] (0,4);
\draw(2,2) to[short] (6,2);
\draw(6,2) to[R, l=$R_L$] (6,-2);
\draw(6,-2) to[short] (-4,-2) to[short] (-4,2) to[short] (-2,2);

\node[circle, draw=black, fill=none, thick, inner sep=2pt] (Vtop) at (9,2) {};
\node[circle, draw=black, fill=none, thick, inner sep=2pt] (Vbot) at (9,-2) {};

\draw (6,2) to[short] (Vtop) node[below] {$+$};
\draw (6,-2) to[short] (Vbot) node[above] {$-$};
...
\noindent
Kurven $V_{out}(t)$ har toppunkt ved $V_{out} = 5 - 1.4 = 3.6\text{V}$. Dette er fordi to dioder må skru på for at strømmen skal kunne passere gjennom lasten.
...
\draw(0,0) to[sV, l={$V_{in}(t)$}] (0,4);
\draw(2,2) to[short] (6,2);
\draw(3.5,2) to[R, l=$R_L$] (3.5,-2);
\draw(3.5,-2) to[short] (-4,-2) to[short] (-4,2) to[short] (-2,2);
\draw(6,2) to[C, l=$C$] (6,-2);

\node[circle, draw=black, fill=none, thick, inner sep=2pt] (Vtop) at (9,2) {};
\node[circle, draw=black, fill=none, thick, inner sep=2pt] (Vbot) at (9,-2) {};

\draw (3.5,2) to[short] (Vtop) node[below] {$+$};
\draw (3.5,-2) to[short] (Vbot) node[above] {$-$};
...
\]
\noindent
Vi integrerer over nullpunktene til funksjonen $10\cdot \sin (t) - 0.7$. Disse er $t = 0.07$ og $t = 3.07$. Perioden er som kjent $T = 0.02$s. Vi regner for DC-verdien.
\[
V_{DC} = \frac{1}{0.02}\int_{0.07}^{3.07} 10\sin (t) - 0.7 \, dt \rightarrow V_{DC} = 2.84\text{V}
\]
