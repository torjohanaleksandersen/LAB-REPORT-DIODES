...
I denne seksjonen presenteres måleresultater for alle kretsene som er koblet opp i forsøket. For hver krets presenteres en kort beskrivelse av fremgangsmåten for oppkobling og måling, etterfulgt av en grafisk fremstilling av målingen på oscilloskopet. I tillegg kommenteres resultatene for hver krets, hvor det gir en kort vurdering av om resultatet er som forventet og om det er noen avvik fra det teoretiske resultatet. Diskusjoner knyttet til resultatene vil bli presentert i diskusjonsseksjonen senere i rapporten.

\subsection{Krets 1 -- enveis klippekrets}
Fremgangsmåten for å koble opp denne kretsen blir å bruke komponentene i krets~\ref{fig:krets1} og koble dem opp på et breadboard. Signalgeneratoren stilles inn på en sinusformet spenning med toppverdi $10\,\mathrm{V}$ og frekvens $50\,\mathrm{Hz}$. Deretter bruker vi en motstand på $180\,\Omega$ i serie med dioden. For å måle klippekretsspenningen, bruker vi oscilloskopet. En kanal kobles til inngangsspenningen, og en annen kanal kobles over dioden. Resultatet av én periode er i figuren under.
...
\noindent
\paragraph{Kommentar til resultatet. } Utgangsspenningen klippes ved $800\,\mathrm{mV}$, som er omtrent det vi forventer for en silisiumdiode. Den negative halvbølgen følger inngangsspenningen. Måleresultatet for denne kretsen er som forventet og har ingen store avvik fra det teoretiske resultatet.

\clearpage

\subsection{Krets 2 -- toveis klippekrets}
For å koble opp kretsen benytter vi komponentene i krets 2 (se figur~\ref{fig:krets2}). Signalgeneratoren bruker innstillingen for en sinusformet spenning med toppverdi $10\,\mathrm{V}$ og frekvens $50\,\mathrm{Hz}$. Vi bruker en motstand på $180\,\Omega$ i serie med to parallellkoblet dioder. Diodene skal være orientert i motstatt retning av hverandre. Oscilloskopet bruker en kanal for å måle inngangsspenningen, og en kanal for å måle over diodene. Måleresultatet er i figur~\ref{fig:resultat_krets2}.
...
\noindent
\paragraph{Kommentar til resultatet. } Utgangsspenningen klippes ved $800\,\mathrm{mV}$ både i den positive og negative halvbølgen, som er det vi forventer for en silisiumdiode. Resultatet her har samme praktiske måling som i krets 1, som betyr at det samsvarer på tvers av kretser. Måleresultatet for denne kretsen er som forventet og har ingen store avvik fra det teoretiske resultatet.

\subsection{Krets 3 -- zenordioder i klippekrets}

Signalgeneratoren bruker innstillingene for en sinusformet spenning med toppverdi $10\,\mathrm{V}$ og frekvens $50\,\mathrm{Hz}$. Vi bruker en motstand på $180\,\Omega$ i serie med to zenordioder. Den ene zenordioden har zenorspenning på $4.7\,\mathrm{V}$ og den andre har zenorspenning på $6.8\,\mathrm{V}$. Diodene skal være plassert og orientert slik i kretsfremstillingen i figur~\ref{fig:krets3}. Oscilloskopet bruker en kanal for å måle inngangsspenningen, og en kanal for å måle over zenordiodene i serie. Måleresultatet er i figur~\ref{fig:resultat_krets3}.
...
\paragraph{Kommentar til resultat. } Utgangsspenningen klippes ved $5.60\,\mathrm{V}$ i den positive halvbølgen og ved $-7.60\,\mathrm{V}$ i den negative halvbølgen. Måleresultatet er som forventet og har ingen store avvik fra det teoretiske resultatet.

\subsection{Krets 4 -- enveis likeretter}

I denne delen av forsøket kobler vi opp krets 4 (se figur~\ref{fig:krets4}). Signalgeneratoren bruker innstillingene for en sinusformet spenning med toppverdi $10\,\mathrm{V}$ og frekvens $50\,\mathrm{Hz}$. Vi bruker en motstand på $1\,\mathrm{k\Omega}$ i serie med en diode. Dioden og motstanden plasseres slik i kretsen. Oscilloskopet bruker en kanal for å måle inngangsspenningen fra signalgeneratoren, og en kanal over motstanden. Måleresultatet for den positive likeretteren kommer når dioden er orientert som i krets 4, og måleresultatet for den negative likeretteren kommer når dioden er snudd i forhold til krets 4. Resultatene er i figur~\ref{fig:resultat_krets4_diode_begge}.
...
\addplot[dashed, gray] coordinates {(0,9) (6.28,9)};
...
\addplot[blue, thick] {10*sin(deg(x))};
\addplot[red, thick] {min(10*sin(deg(x)) +0.6 - 0.18*sin(deg(x)), 0)};

\addplot[dashed, gray] coordinates {(0,-9.28) (6.28,-9.28)};
...
% Klippelinjer
\addplot[dashed, gray] coordinates {(0,-0.8) (6.28,-0.6)};
...
\paragraph{Kommentar til resultater. } Grafen for utgangsspenningen er ikke som forventet. Den oppfører seg som en enveis likeretter, hvor også den negative halvbølgen klippes ved $-600\,\mathrm{mV}$. Dette skyldes jordingsfeil i kretsen, og resultatene har et forventet avvik. Ved å benytte en transformator som inngangsspenning, vil vi kunne måle resultatet for en toveis likeretter uten jordingsfeil.

\paragraph{Bruk av transformator. } Fremgangsmåten for å koble opp krets 5 med transformator er i utgangspunktet lik som uten transformator. Forskjellen er at signalgeneratoren ikke benyttes. I stedet kobler vi en transformator til strømnettet, og bruker den som inngangsspenning for kretsen. For å måle på kretsen må vi passe på jordingsforholdene. Vi kan ikke måle innngangsspenningen samtidig som utgangsspenningen, da dette vil skape en jordingsfeil. I dette forsøket er inngangsspenning og utgangsspenning målt hver for seg og tegnet i samme graf.
...
\addplot[
    dashed,
    gray
] coordinates {(0,18.8) (6.28,18.8)};

\addplot[
    dashed,
    gray
] coordinates {(0,20.8) (6.28,20.8)};
...
\paragraph{Kommentar til resultater ved bruk av transformator. } Resultatene ved bruk av transformator som inngangsspenning er mer som forventet, da jordingsfeilene er eliminert. Vi kan også merke oss av et noe endret inngangssignal. Frekvensen er den samme på $50\,\text{Hz}$, men amplituden er nå rundt $20.8\,\mathrm{V}$ i stedet for $10\,\mathrm{V}$. Verdiene kan ikke sammenliknes direkte med teoriseksjonen for denne kretsen, på grunn av en endret inngangsspenning. Uansett, kan vi se at den karakteristiske toveis likeretterkurven er som forventet, med et spenningstap på $2.0\,\mathrm{V}$ over to dioder. DC-verdien er også målt til $11.17\,\mathrm{V}$, som er høyere enn det teoretiske, men det er rimelig med en høyere inngangsspenning. Resultatene her er uten store avvik og er forventet.
...
\begin{axis}[
    ...
    samples=500,
    domain=0:15.7,
    ytick={0,10,19.36,20.8},
    yticklabels={$0$, $10$, $19.36$, $20.8$},
...
\addplot[
    dashed,
    gray
] coordinates {(0,19.36) (15.7,19.36)};
...
\begin{axis}[
    ...
    samples=500,
    domain=0:15.7,
    ytick={0,10,13.2,20.8},
    yticklabels={$0$, $10$, $13.2$, $20.8$},
    xtick={3.14,6.28,9.42,12.56,15.7},
    xticklabels={$10$, $20$, $30$, $40$, $50$},
    legend style={
        at={(0.98,0.98)},
...
% Rød: med kondensator, første sinusdel fram til tangentpunkt
\addplot[
    red,
    thick,
    domain=0:1.740
...
% Periode 2
\addplot[red, thick, domain=3*pi/2:4.882] {abs(20.8*sin(deg(x)))};
\addplot[red, thick] coordinates {
    (4.882, 20.504)
    (6.971, 13.2)
};
\addplot[red, thick, domain=6.971:5*pi/2] {abs(20.8*sin(deg(x)))};

% Periode 3
\addplot[red, thick, domain=5*pi/2:8.023] {abs(20.8*sin(deg(x)))};
\addplot[red, thick] coordinates {
    (8.023, 20.504)
    (10.112, 13.2)
};
\addplot[red, thick, domain=10.112:7*pi/2] {abs(20.8*sin(deg(x)))};

% Periode 4
\addplot[red, thick, domain=7*pi/2:11.165] {abs(20.8*sin(deg(x)))};
\addplot[red, thick] coordinates {
    (11.165, 20.504)
    (13.254, 13.2)
};
\addplot[red, thick, domain=13.254:9*pi/2] {abs(20.8*sin(deg(x)))};

% Hjelpelinje
\addplot[
    dashed,
    gray
] coordinates {(0,13.2) (15.7,13.2)};
...
\paragraph{Kommentar til resultater. } Resultatene er som forventet. Vi kan tydelig se at ved høyere kapasitans, så er ripplet mindre og DC-verdien høyere. Grafene og teoretiske verdier samsvarer med hverandre. Resultatene er uten store avvik og er som forventet.
